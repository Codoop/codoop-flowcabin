---
name: codoop-game
description: Create, iterate on, preview, validate, and package a resumable offline H5 game for Codoop from a natural-language game idea. Use when a creator wants to make a Codoop/Flow Cabin game, change its player experience, or deliver game.zip and a separate cover.png.
---
...
3. Create an isolated project with `scripts/create-game.mjs <workspace> <name>`.
   Use the Canvas starter by default. Build only local static resources and use
   the Flow Cabin API in `references/flow-cabin-api.md`.
4. Start `scripts/preview-harness.mjs <project>` and give the creator its local
   preview URL. Run lifecycle, resize, input, and offline checks before saying
...
5. For each feedback round, write the changed hypothesis to `design-notes.md`,
   ...
   preview again.
6. On approval, get `cover.png` from the creator first. If they ask you to
...
7. Re-read all triggered expert files for final approval. Run `validate-game`,
